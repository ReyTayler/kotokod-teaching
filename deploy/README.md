...
**Адрес журнала для бота — `http://127.0.0.1:8001`, а не :8000.** Gunicorn слушает
unix-сокет, TCP-порта у него нет вовсе; :8000 — это адрес `runserver` и работает
только на машине разработчика. Для бота в nginx поднят отдельный слушатель на
петлевом интерфейсе (`listen 127.0.0.1:8001`), обслуживающий только
`/api/integrations/`. Снаружи он недостижим физически. В `.env` бота:
`JOURNAL_API_URL=http://127.0.0.1:8001`.

**Telegram-уведомления. Сначала миграции — иначе ляжет раздел преподавателей.**
Приложение `notifications` создаёт три новые таблицы, и `list_teachers` читает
одну из них (привязку). Без миграций `/admin/teachers` отдаёт 500 — ошибка
выглядит совершенно не связанной с уведомлениями, и на неё легко потратить час:

```bash
cd /opt/kotokod/journal-backend/journal_django
.venv/bin/python manage.py migrate
```

Тесты этого не ловят: они всегда работают на свежесмигрированной тестовой БД.

`journal-celery-beat` несёт три дополнительных расписания: разгребание очереди (раз в минуту), утренний дайджест расписания
