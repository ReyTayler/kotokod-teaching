...
- `repository.preview_affected(...)` — read-only зеркало `wipe_one_offs`: список
  ...
  подтверждением («Изменить расписание», заморозка).

Заморозка/разморозка ученика по-прежнему перекладывают хвост через
`planner.relay_from_date(..., skip_dates=...)` (непрерывная раскладка по
открытому слоту, обходя занятые даты) — единственный оставшийся потребитель
этого примитива после переработки отмены/переноса-навсегда.
...
Заморозка ученика (`POST /api/admin/students/<id>/status` с `status=frozen`) и её
превью (`POST .../status/preview`) — вне `scheduling`-эндпоинтов (см. `apps.students`),
но используют те же примитивы: `wipe_one_offs` перед перекладкой хвоста, `affected`
в ответе превью (`preview_affected`) на каждое индив-членство.
