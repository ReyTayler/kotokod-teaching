...
**Важно про сборку.** В этом репозитории `journal_django/frontend/admin-dist/` **отслеживается
git'ом** (33 файла, в `.gitignore` его нет) — именно его раздаёт nginx. Поэтому после правок фронта
`npm run build` нужен, и пересобранный бандл коммитится отдельным коммитом, не вперемешку с
исходниками. Внутри задачи собирать не надо: сборка делается один раз в конце, когда весь фронт
готов, иначе диф засоряется десятком хешированных файлов на каждом шаге.
