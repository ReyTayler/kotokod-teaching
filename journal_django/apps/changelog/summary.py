...
FIELD_RU = {
    ...
    # LEGACY: поля ученика удалены (спека 2026-07-25), но подписи нужны для уже
    # записанных событий — иначе в старой ленте вместо «статус зачисления»
    # покажется сырой ключ (FIELD_RU — фолбэк «как есть»).
