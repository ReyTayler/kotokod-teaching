...
def test_revert_endpoint_double_revert_400(admin_client):
...
def test_revert_endpoint_revert_of_revert_400(admin_client):
    ...
    revert_op_id = next(
        r['id'] for r in admin_client.get('/api/admin/changelog?page_size=5').json()['rows']
        if r['operation'] == 'changelog.revert'
    )
    resp = admin_client.post(f'/api/admin/changelog/{revert_op_id}/revert')
    assert resp.status_code == 400
    assert resp.json()['error']
