...
def resume_student(student_id: int, *, actual_resume_date, actor=None) -> bool:
    ...
    #
    # ВАЖНО: фильтр group__is_individual цепляет ВСЕ индив-группы, где ученик
    # когда-либо был, включая давно-завершённые курсы (active=False по причине
    # окончания курса, а не этой заморозки). Реактивировать их скопом нельзя —
    # это воскресило бы неактуальные членства. Поэтому реактивируем ТОЛЬКО те
    # группы, чей resume реально переложил курсовой хвост (>0): у настоящей
    # замороженной группы хвост в окне есть, у завершённого курса — нет (все done
    # / нет строк), resume_individual_group вернёт 0.
