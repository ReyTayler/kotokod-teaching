...
from apps.extra_lessons.models import CANCELLED, DONE, SCHEDULED, ExtraLessonAssignment
...
def test_create_assignment_raises_if_student_not_absent(
    teacher_fixture, missed_lesson_fixture,
):
    ...
    try:
        ...
        assert not ExtraLessonAssignment.objects.filter(
            missed_lesson_id=missed_lesson_fixture,
        ).exists()
    finally:
        with connection.cursor() as cur:
...
def test_record_raises_if_student_balance_dropped_since_assignment(
    ...
    assert ExtraLessonAssignment.objects.get(id=created['id']).status == SCHEDULED
...
def test_delete_fact_second_call_raises_value_error(
    ...
    """
    Регрессия (audit, race condition): после успешного delete_fact() назначение
    вернулось в status=scheduled — повторный вызов delete_fact() на том же
    assignment_id обязан упасть ValueError'ом. Проверяет авторитетную проверку
    статуса под select_for_update() внутри atomic() (repository.
    lock_assignment_for_delete) — тот же паттерн, что record()/
    lock_assignment_for_record, иначе гонка двух параллельных delete_fact()
    привела бы ко второму Lesson.DoesNotExist вместо чистой ошибки.
    """
    created = services.create_assignment(
        {
            'missed_lesson_id': missed_lesson_fixture, 'teacher_id': teacher_fixture,
    ...
    assert ok is True
    with pytest.raises(ValueError):
        services.delete_fact(created['id'], _FakeRequest())
